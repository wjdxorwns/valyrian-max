...
CREATE TABLE Users(
	...
	generation_day DATETIME  NOT NULL DEFAULT NOW(),		  -- 생성일
...
CREATE TABLE Request(                                       -- 대기: 프론트에서 onclick 이벤트 실행 전 단계
	...
	request_salary TINYINT NOT NULL DEFAULT 0,                                 -- 급여 CRUD 관련 승인 요청 기능
	request_vacation TINYINT NOT NULL DEFAULT 0,                               -- 근휴가 관련 승인 요청 기능
	request_worktype TINYINT NOT NULL DEFAULT 0,                               -- 근무 방식 관련 승인 요청 기능
	reject TEXT,                                            -- 거절 시 관리자가 그에 대한 사유를 적는 곳
	FOREIGN KEY (emp_idx) REFERENCES employee(emp_idx)
);
...
CREATE TABLE RequestLogging(
	...
	logging_salary VARCHAR(10),                             -- 급여 권한 승인 기능
	logging_vacation VARCHAR(10),                           -- 휴가 권한 승인 기능
	logging_worktype VARCHAR(10),                           -- 근무 방식 권한 승인 기능
	reject TEXT,                                            -- 거절 사유
...
CREATE TABLE Vacation (
	...
	request_idx INT,                                   	  -- 직원 코드
	vacation_type ENUM('연차','공가') NOT NULL,             -- 휴가 종류
	start_date DATE,                                        -- 시작일
	end_date DATE,                                          -- 종료일
