			...
			<ul class="nav-menu">
					...
					<ul class="dropdown-menu">
						<c:choose>
							<c:when test='${sessionScope.role == "슈퍼관리자"}'>
								...
								<li>
									<a href="/PermissionRequest?emp_idx=${sessionScope.employeeVO.emp_idx}">요청 승인1</a>
								</li>
							</c:when>
							
							<c:when test='${sessionScope.role == "관리자"}'>
									<li>
										<a href="/PermissionRequest?emp_idx=${sessionScope.employeeVO.emp_idx}">요청 승인2</a>
									</li>
							</c:when>
							
							<c:otherwise>
								<li>
									<a href="/PermissionRequest?emp_idx=${sessionScope.employeeVO.emp_idx}">요청 승인3</a>
								</li>
							</c:otherwise>						
						</c:choose>
					</ul>
				...
				<c:if test="${not empty sessionScope.userVO}">
					<li class="nav-item user-info"><a href="#">${sessionScope.userVO.emp_name}님
							환영합니다!</a>
						<ul class="dropdown-menu">
							<li><a href="/myPage?emp_idx=${sessionScope.employeeVO.emp_idx}">내 정보</a></li>
