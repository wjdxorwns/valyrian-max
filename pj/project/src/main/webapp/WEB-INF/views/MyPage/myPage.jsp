...
<main>
		...
		<table>
			<tbody>
				...
				<tr>
					...
					<th>성별</th>
					<td>
						<input type="text" class="box2" value="${eVO.gender == 0 ? '남자' : '여자'}" readonly />
					</td>
				</tr>
				<tr>
	...
	<div>
		...
		<p>Sign Name: ${eVO.signature}</p>
		<p>Sign Path: ${eVO.sign_path}</p>
		
		<!-- 이미 저장된 사인 이미지 표시 -->
		<c:if test="${not empty eVO.signature}">
			<img src="${pageContext.request.contextPath}${eVO.signature}" 
				 alt="사인 이미지"
				 onerror="console.log('이미지 로드 실패:', this.src);">
		</c:if>
	</div>
</main>
	<!-- Footer -->
