...
public class ManagerController {
		...
		public ModelAndView registerEmployee(
			...
			try {
				...
				managerService.registerEmployee(employeeVO, userVO, usersignVO, requestVO, permissionVO);
				
				/// 사진이 업로드되었을 경우만 처리
				if (profileImg != null && !profileImg.isEmpty()) {
					String fileName = UUID.randomUUID().toString() + "_" + profileImg.getOriginalFilename();
					// 웹 접근 가능한 경로로 수정
					String uploadDir = request.getSession().getServletContext().getRealPath("/resources/images/");
					...
					picVO.setF_path("/resources/images/" + fileName);
				 ...
				 // 등록 후 전체 직원 목록 조회
		        mv.addObject("employeeList", managerService.getAllEmployees(null, null));
		        mv.addObject("param", Map.of(
		            "searchType", "",
		            "keyword", ""
		        ));
				...
				mv.addObject("message", "직원이 성공적으로 등록되었습니다.");
		        return mv;  // ModelAndView 객체를 반환
		    } catch (Exception e) {
		        e.printStackTrace();
		        mv.addObject("error", "직원 등록 중 오류가 발생했습니다.");
		        mv.setViewName("PersonnelManagement/EmpManagement/empDataRegister");
		        return mv;
		    }
		}
		...
		//직원 상세조회
				...
				public ModelAndView getEmployeeDetail(@RequestParam("emp_idx") String emp_idx) {
				    ...
				    System.out.println("직원 상세 정보: " + employeeData);
				    System.out.println("f_name: " + employeeData.get("f_name"));
				    System.out.println("f_path: " + employeeData.get("f_path"));
		...
		public ModelAndView updateEmployee(
		    ...
		    try {
		        ...
		        if (phone1 != null && phone2 != null && phone3 != null) {
		            String combinedPhone = phone1 + "-" + phone2 + "-" + phone3;
		            param.put("phone_number", combinedPhone);
		        }
		        ...
		        if ("관리자".equals(employeeVO.getDept_name()) || "슈퍼관리자".equals(employeeVO.getDept_name())) {
		            System.out.println("관리자 권한 추가 시도");
		            managerService.addPermissionIfNotExists(employeeVO.getEmp_idx());
		        } else {
		            System.out.println("관리자 권한 제거 시도");
		        ...
		        if (profileImg != null && !profileImg.isEmpty()) {
		         ...
		         // 로그 추가
		            ...
		            empPictureVO.setF_path("/resources/images/" + fileName);
		            empPictureVO.setF_width(180); 
		            empPictureVO.setF_height(200);

		            // 사진 DB "수정"
		            try {
		                // 먼저 update 시도
		                int result = managerService.updateProfileImage(empPictureVO);
		                
		                // update가 실패하면(affected rows가 0이면) insert 시도
		                if(result == 0) {
		                    managerService.insertEmpPicture(empPictureVO);
		                }
		            } catch(Exception e) {
		                System.out.println("프로필 이미지 처리 중 오류: " + e.getMessage());
		                e.printStackTrace();
		            }
		        }
		        
		        // 수정 후 전체 직원 목록 조회
		        List<Map<String, Object>> employeeList = managerService.getAllEmployees(null, null);
		        mv.addObject("employeeList", managerService.getAllEmployees(null, null));
		        mv.addObject("param", Map.of(
		            "searchType", "",
		            "keyword", ""
		        ));
		        mv.addObject("message", "직원 정보가 성공적으로 수정되었습니다.");
		        return mv;
		    } catch (Exception e) {
		        e.printStackTrace();
		        mv.addObject("error", "직원 정보 수정 중 오류가 발생했습니다.");
		    }
		    return mv;
		}
		
		// 퇴사 직원 리스트 블라 처리
		@GetMapping("/PersonnelManagement/ResignManagement/resignDataList")
		public ModelAndView getResignEmployeeList() {
		    ModelAndView mv = new ModelAndView("PersonnelManagement/ResignManagement/resignDataList");
		    mv.addObject("employeeList", managerService.getResignEmployees());
		    return mv;
		}
