...
public class NorLoginAndRegisterController {
    ...
    public ModelAndView login(@RequestParam String emp_email, @RequestParam String emp_password, HttpSession session) {
        ...
        try {
            ...
            if (user == null) {
            	logger.warn("User not found for email: {}", emp_email);
                mv.addObject("error", "이메일이 등록되지 않았습니다.");
            ...
            if (!passwordEncoder.matches(emp_password, user.getEmp_password())) {
            	logger.warn("Password mismatch for email: {}", emp_email);
            ...
            if (employee == null) {
            	logger.warn("Employee not found for user_idx: {}", user_idx);
                mv.addObject("error", "직원 정보가 존재하지 않습니다.");
                mv.setViewName("MainPage/login");
                return mv;
            }
            ...
            logger.info("Login successful for email: {}", emp_email);
