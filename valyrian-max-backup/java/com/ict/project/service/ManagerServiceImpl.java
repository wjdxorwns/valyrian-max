...
public class ManagerServiceImpl implements ManagerService{
    ...
    public int updateProfileImage(EmpPictureVO picture) {
        return managerDAO.updateProfileImage(picture);
    }

    @Override
    public void insertEmpPicture(EmpPictureVO picVO) throws Exception {
        if (picVO.getEmp_idx() == null || picVO.getEmp_idx().isEmpty()) {
            throw new IllegalArgumentException("직원번호(emp_idx)가 비어 있습니다.");
        }
        
        // employee 테이블에 해당 emp_idx가 존재하는지 확인
        if (managerDAO.checkEmpIdxDuplicate(picVO.getEmp_idx()) == 0) {
            throw new IllegalArgumentException("해당 직원번호(" + picVO.getEmp_idx() + ")가 employee 테이블에 존재하지 않습니다.");
        }
    ...
    public void removePermission(String emp_idx) {
    	System.out.println("removePermission called with emp_idx: " + emp_idx);
    ...
    public List<Map<String, Object>> getResignEmployees() {
        return managerDAO.getResignEmployees();
    }
