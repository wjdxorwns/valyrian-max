...
<body>
		...
		<div class="profile-container">
			...
			<form
				...
				<div class="profile-header">
					...
					<div class="section">
						<input type="hidden" name="emp_idx"
							value="${employeeData.emp_idx}"> <input type="hidden"
							name="user_idx" value="${employeeData.user_idx}">
						<div class="row">
						...
						<div class="row00">
							<label>권한부여</label> <select name="dept_name" id="dept_name">
								<option value="대기"
	...
	<script type="text/javascript">
		...
		function validateForm() {
			...
			if (domain === "custom") {
				domain = $("#custom_domain").val();
			}
			// ★★★ 이 부분이 항상 실행되어야 함 ★★★
			$("#emp_email").val(emailId && domain ? (emailId + domain) : "");
			...
			$("#phone_number")
					.val(
							(phone1 && phone2 && phone3) ? (phone1 + "-"
									+ phone2 + "-" + phone3) : "");

			// 중복체크는 변경된 경우에만
			if (emailId
					&& emailId !== "${fn:split(employeeData.emp_email, '@')[0]}") {
				if ($("#email_check_result").text() === "이미 사용중인 이메일입니다.") {
					alert("이미 사용중인 이메일입니다.");
					return false;
				}
			}
			const currentPhone = phone1 + "-" + phone2 + "-" + phone3;
			const originalPhone = "${employeeData.phone_number}";
			if (currentPhone !== originalPhone) {
				if ($("#phone_check_result").text() === "이미 사용중인 전화번호입니다.") {
					alert("이미 사용중인 전화번호입니다.");
					return false;
				}
			}
			var quitter = document.getElementById('quitter').value;
			var resignDate = document.getElementById('resignation_date').value;
			if (quitter === '퇴사대기' && !resignDate) {
				alert('퇴사일자를 입력하세요.');
				return false;
			}
			return true;
		...
		function toggleResignDate() {
			var quitter = document.getElementById('quitter').value;
			var resignDate = document.getElementById('resignation_date');
			if (quitter === '퇴사대기') {
