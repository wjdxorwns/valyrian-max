...
<body>
	...
	<main>
		...
		<section class="notice-list">
			...
			<c:forEach var="notice" items="${noticeList}">
				<c:if test="${notice.importance == '중요'}">
					<div class="notice-item important"
						data-notice-id="${notice.board_id}" onclick="goToDetail(this)">
						<div class="user-icon">${notice.username.charAt(0)}</div>
						<div class="content-wrapper">
							<div class="user-info">
								<span class="username">${notice.username}</span> <span
									class="date">${notice.created_at}</span> <span
									class="importance-badge">중요</span>
							</div>
							<div class="post-content">
								...
								<c:if test="${not empty notice.f_name}">
									<img
										src="${pageContext.request.contextPath}/Uploads/${notice.f_name}"
										alt="공지사항 이미지" class="post-image">
								</c:if>
							</div>
							...
							<c:if test="${sessionScope['user.admin'] == 'ok'}">
								<div class="action-buttons">
									<button class="edit-button"
										onclick="goToEdit('${notice.board_id}'); event.stopPropagation();">
										수정</button>
								</div>
							</c:if>
						</div>
			...
			<c:forEach var="notice" items="${noticeList}">
				<c:if test="${notice.importance != '중요'}">
					<div class="notice-item" data-notice-id="${notice.board_id}"
						...
						<div class="content-wrapper">
							<div class="user-info">
								<span class="username">${notice.username}</span> <span
									class="date">${notice.created_at}</span> <span
									class="importance-badge normal">일반</span>
							</div>
							<div class="post-content">
								<span>${notice.title}</span>
								<p>${notice.content}</p>
