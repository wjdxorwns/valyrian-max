		...
		<h2>직원 근무지 이동 신청 사항</h2>
			...
			<div id="standby">
				<p>근무지 이동 신청 목록</p>
					...
					<tbody>
						<c:choose>
							<c:when test="${empty applyChangeList}">
								<tr>
									<td colspan="9" class="no-data">근무지 이동 신청 내역이 없습니다.</td>
								</tr>
							</c:when>
							<c:otherwise>
								<c:forEach var="applyChange" items="${applyChangeList}">
									<c:if test="${applyChange.logging_personnelchange == null}">
								
										<% waitCount++; %>
										<tr onclick="showModal(parseInt('${applyChange.request_idx}'), parseInt('${applyChange.logging_idx}'), event)">
											<td><%= waitCount %></td>
											<td>${applyChange.emp_id}</td>
											<td>${applyChange.team}</td>
											<td>${applyChange.position}</td>
											<td>${applyChange.emp_name}</td>
											<td>${applyChange.emp_email}</td>
											<td>${applyChange.prev_location_id}</td>
											<td>${applyChange.new_location_id}</td>
											<td>대기</td>
											<td>${applyChange.request_text}</td>
										</tr>
										<% System.out.println("Debug - applyChange data: " + pageContext.getAttribute("applyChange")); %>
									</c:if>
								</c:forEach>
							</c:otherwise>
						</c:choose>
						...
						<c:choose>
							...
							<c:otherwise>
								<c:forEach var="request" items="${personChangeList}">
									<c:if test="${request.logging_personnelchange == '승인'}">
										<% endorseCount++; %>
										<tr onclick="showModal(parseInt('${request.request_idx}'), parseInt('${request.logging_idx}'), event)">
											<td><%= endorseCount %></td>
											...
											<td>${request.prev_location_id}</td>
											<td>${request.new_location_id}</td>
											<td>승인</td>
				...
				<table>
					...
					<tbody>
						<c:choose>
							...
							<c:otherwise>
								<c:forEach var="request" items="${personChangeList}">
									<c:if test="${request.logging_personnelchange == '반려'}">
										<% declineCount++; %>
										<tr onclick="showModal(parseInt('${request.request_idx}'), parseInt('${request.logging_idx}'), event)">
											<td><%= declineCount %></td>
											<td>${request.emp_id}</td>
			...
			<div id="personChange_detail" class="modal">
				<div class="detail-content">
					...
					<input type="hidden" id="modal_logging_idx" value="">
					<div id="title">
						<h2>근무지 이동 신청 상세</h2>
					</div>
					<table>
						<tbody>
							...
							<tr>
								<th>이전 근무지</th>
								<td id="detail_prev_location"></td>
							</tr>
							<tr>
								<th>새 근무지</th>
								<td id="detail_new_location"></td>
							</tr>
							<tr>
								<th>결제</th>
	...
	<script type="text/javascript">
		...
		document.querySelectorAll("#standby tbody tr").forEach(row => {
			row.addEventListener("click", function() {
				...
				document.getElementById("detail_prev_location").textContent = cells[6].textContent;
				document.getElementById("detail_new_location").textContent = cells[7].textContent;
		...
		function Approval_ok() {
			const requestData = {
				...
				logging_personnelchange: document.querySelector("select[name='modal_logging_personnelchange']").value,
		...
		function showModal(requestIdx, loggingIdx, event) {
			const row = event.target.closest('tr');
			const cells = row.getElementsByTagName("td");
		
			// request_idx와 logging_idx를 hidden input에 저장
			document.getElementById("modal_request_idx").value = requestIdx;
			document.getElementById("modal_logging_idx").value = loggingIdx || '';
