...
public class NorLoginAndRegisterController {
	...
	public ModelAndView login(@RequestParam String emp_email, @RequestParam String emp_password, HttpSession session) {
		...
		try {
			...
			session.setAttribute("emp_name", user.getEmp_name());
	...
	// AJAX 요청에 대한 응답을 위해 @ResponseBody (또는 클래스 레벨에 @RestController)와 ResponseEntity
	// 사용
	...
	public ResponseEntity<String> updateProfile(@RequestBody Map<String, String> payload, HttpSession session) {
		// @RequestBody Map<String, String> payload : JSON {"password": "...",
		// "signature": "..."} 형태의 데이터를 받음
		// 또는 아래처럼 ProfileUpdateDTO 같은 객체를 만들어 받을 수도 있습니다.
		// public ResponseEntity<String> updateProfile(@RequestBody ProfileUpdateDTO
		// payload, HttpSession session) {
		...
		try {
			...
			logger.error("Profile update error for user_idx {}: ",
					(session.getAttribute("userVO") != null ? ((UsersVO) session.getAttribute("userVO")).getUser_idx()
							: "Unknown"),
					e);
