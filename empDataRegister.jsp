...
<body>
					...
					<div class="section">
						<div class="row">
							...
							<label>직원번호</label> <input type="text" name="emp_id" id="emp_id"
								value="" placeholder="고용타입 선택 시 자동 생성" readonly>
						</div>
						<div class="row">
						...
						<div class="row">
								...
								<label>근무방식</label>
								<select name="attitude_type" id="attitude_type">
						...
						<div class="row0">
							...
							<span id="email_check_result" style="color: #666; margin-left: 10px;"></span>
							</div>
							<div class="row00">
						...
						<tr>
							...
							<td>
								<div class="phone-input-group">
									<input type="text" name="phone1" id="phone1" maxlength="3" placeholder="010" required>
									<span>-</span>
								...
								<span id="phone_check_result" style="color: #666; margin-left: 10px;"></span>
							</td>
						</tr>
						<tr>
							...
							<td colspan="3"><input id="addr" name="detail_home"
								id="detail_home" value="" placeholder="상세주소 입력" style="width:100%"></td>
						</tr>
						<tr>
	...
	function checkEmailDuplicate() {
	    ...
	    // 중복 확인 AJAX 요청
	    $.ajax({
	        url: "${pageContext.request.contextPath}/manager/checkDuplicate",
	        type: "GET",
	        data: {
	            field: "emp_email",
	            value: fullEmail
	        ...
	        success: function(response) {
	            if (response.error) {
	                resultSpan.text(response.error).css("color", "red");
	            } else if (response.duplicate) {
	                resultSpan.text("이미 사용중인 이메일입니다.").css("color", "red");
	            } else {
	                resultSpan.text("사용 가능한 이메일입니다.").css("color", "green");
	            }
	        },
	        error: function() {
	            resultSpan.text("중복확인 중 오류가 발생했습니다.").css("color", "red");
	        }
	    });
	}
	    ...
	    $.ajax({
	        ...
	        success: function(response) {
	            if (response.error) {
	                ...
	                resultSpan.text("이미 사용중인 전화번호입니다.").css("color", "red");
	            } else {
	                resultSpan.text("사용 가능한 전화번호입니다.").css("color", "green");
	            }
	        },
	        error: function() {
	            resultSpan.text("중복확인 중 오류가 발생했습니다.").css("color", "red");
